...
def clean_adduct(adduct, add_brackets=True, add_missing_charge=True, replace_names_by_formulas=False,
    ...
    for i in reversed(range(len(new_adduct))):
        if new_adduct[i] == "+" or new_adduct[i] == "-":
            ...
            else:  # charge detected - remove from str
                new_adduct = new_adduct[0:i + 1]
            break
...
if __name__ == '__main__':
    ...
    # list2 = ["M+ACN+Fa", "M+H","[M+H]","[M+H]+", "M+H+", "M-H", "M-H-", "M+2H", "M+H-1H2O", "M+H-2H2O", "M+H-3H2O",
    #        "M+Na", "2M+Na", "[2M+Na]+", "M+2Na-H", "M+Ca-H", "M+Ca", "M+2H+ACN-2H2O", "M+Cl", "M+Fa", "M+Na+2Cl",
    #        "Cat", "Cat+H", "Cat-2H", "M++", "M+3H+++", "M-3H---", "M2+H", "M+ACN+Fa", "Unk+", "?"]
